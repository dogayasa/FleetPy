...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "AutonomousOnly", "scenarios")
   
        """
        EXPERIMENTS FOR AUTONOMOUS FLEET 
        1 - Base fleet scenario for 8 days.
        ...
        # 1 - Base fleet scenario for 8 days. ----------------------------------------
        log_level = "info"
        cc = os.path.join(scs_path, "0_const_AO.csv")
        sc = os.path.join(scs_path, "0_base_8d_AO.csv")
